...
export function SecretaryVerificationView() {
  ...
  const countLabel = useMemo(() => query.isLoading ? "Loading" : `${pagination?.total ?? 0} projects`, [pagination?.total, query.isLoading]);

  return (
    <div className="mx-auto flex h-full w-full flex-col p-6 lg:p-8">
      <div className="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <div className="mb-2 flex items-center gap-2 text-sm font-bold text-primary"><ClipboardCheck className="h-4 w-4" /> Secretary Review</div>
          <h1 className="text-3xl font-extrabold tracking-tight text-[#191c20]">Project Screening</h1>
          <p className="mt-1 text-sm text-[#3f4942]">Review submitted projects from every department.</p>
        </div>
      ...
      <Card className="flex flex-1 flex-col overflow-hidden rounded-md border-[#D1CDC7] shadow-sm">
        <div className="flex flex-col gap-3 border-b border-[#ededf4] p-6 sm:flex-row sm:items-center sm:justify-between sm:px-10">
          <div>
            <h2 className="text-lg font-extrabold text-[#191c20]">Pending Secretary Review</h2>
            <p className="mt-1 text-sm text-muted-foreground">{countLabel}</p>
          </div>
          <div className="relative w-full sm:max-w-md">
            <Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
            <Input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Search project code, name, or owner" className="h-11 rounded-full pl-10" />
          </div>
        </div>

        {query.isError ? (
          <div className="flex flex-col items-center gap-3 p-16 text-center text-destructive">
            <p className="font-semibold">Unable to load pending projects.</p>
            <p className="text-sm">{query.error instanceof Error ? query.error.message : "Please try again."}</p>
          ...
          <>
            <ProjectTable
              data={projects}
              activeTab="all"
              emptyMessage="No projects are waiting for Secretary review."
