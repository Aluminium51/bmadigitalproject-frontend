...
import { MoreVertical, FileSpreadsheet, FileText, Clock, CheckCircle2, AlertCircle } from "lucide-react";
      ...
      <table className="w-full text-left text-sm whitespace-nowrap">
        <thead className="bg-white sticky top-0 text-slate-400 font-bold z-10 border-b border-[#ededf4] text-[13px] uppercase tracking-wide">
          <tr>
            ...
            <th className="px-6 sm:px-10 py-4 min-w-50">{activeTab === "drafts" ? "ความคืบหน้า" : "สถานะโครงการ"}</th>
        ...
        <tbody className="divide-y divide-[#ededf4]">
          {data.map((project) => {
            ...
            const importDate = project.createdAt ? new Date(project.createdAt).toLocaleDateString('th-TH') : "-";

            return (
              <tr key={project.id} className={rowClass} onClick={() => router.push(`/projects/${project.id}`)}>

                {/* วันที่นำเข้า */}
                <td className="px-6 sm:px-10 py-5 text-xs text-muted-foreground">{importDate}</td>

                {/* ชื่อโครงการ */}
                <td className={`px-6 sm:px-10 py-5 font-bold flex flex-col ${isReturned && activeTab !== "drafts" ? "text-red-700" : "text-[#191c20] group-hover:text-[#00734b]"}`}>
                ...
                <td className="px-6 sm:px-10 py-5 text-[#3f4942]">
                  {project.initialRequestedBudget ? parseFloat(project.initialRequestedBudget).toLocaleString('th-TH') : "-"}
                </td>

                {/* ผู้วิเคราะห์ */}
                <td className="px-6 sm:px-10 py-5 text-[#3f4942]">{project.analystId || "-"}</td>
                ...
                <td className="px-6 sm:px-10 py-5 text-center" onClick={(e) => e.stopPropagation()}>
