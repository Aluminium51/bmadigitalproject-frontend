...
export default function ProjectWorkspacePage() {
  ...
  const [diagrams, setDiagrams] = useState<DocumentFile[]>([
    { id: "1", name: "System_Architecture.png", type: "image", size: "2.4 MB" }
  ]);
  const [presentation, setPresentation] = useState<DocumentFile | null>(null);
  const [quotation, setQuotation] = useState<DocumentFile | null>({ id: "q1", name: "ใบเสนอราคา_บ.เอบีซี.pdf", type: "pdf", size: "1.2 MB" });
    ...
    return (
      <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 hover:bg-slate-50/50 transition-colors gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">รองรับไฟล์ {accept.replace(/\./g, " ").toUpperCase()}</p>
        </div>
    ...
    <div className="flex items-center justify-between p-4 bg-surface rounded-[20px] border border-[#D1CDC7] group hover:border-[#00734b]/40 transition-all shadow-sm">
      <div className="flex items-center gap-3 overflow-hidden">
        ...
        <div className="flex flex-col truncate">
          <span className="text-sm font-bold text-[#191c20] truncate">{file.name}</span>
          {file.size && <span className="text-[11px] text-muted-foreground mt-0.5">{file.size}</span>}
        </div>
      </div>
      <button onClick={onRemove} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-full opacity-0 group-hover:opacity-100 transition-all shrink-0">
        <Trash2 className="w-4 h-4" />
      </button>
    </div>
  );
  ...
  const DiagramUploadPlaceholder = () => (
    <div className="border-2 border-dashed border-[#D1CDC7] rounded-[20px] p-6 flex flex-col items-center justify-center text-center bg-white hover:bg-slate-50 hover:border-[#00734b]/50 transition-colors cursor-pointer group h-full min-h-[120px]">
      ...
      <span className="text-sm font-bold text-[#3f4942] group-hover:text-[#191c20] transition-colors">เพิ่ม Diagram</span>
      <span className="text-xs text-muted-foreground mt-1">รองรับ PNG, JPG</span>
    ...
    <div className="p-4 sm:p-6 lg:p-8 flex flex-col gap-8 max-w-7xl mx-auto w-full animate-in fade-in duration-500">
      ...
      <div className="bg-white p-6 md:p-10 rounded-container border border-[#D1CDC7] shadow-sm flex flex-col gap-6 relative overflow-hidden">
        ...
        <div className="flex justify-between items-start gap-4 z-10">
          ...
          <Button variant="outline" size="sm" onClick={() => setIsEditingInfo(!isEditingInfo)} className="rounded-full border-[1.5px] font-bold shrink-0 mt-1">
            {isEditingInfo ? <><CheckCircle2 className="w-4 h-4 mr-2"/> บันทึก</> : <><Edit2 className="w-4 h-4 mr-2"/> แก้ไข</>}
          </Button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 z-10 p-5 bg-surface/50 rounded-[24px]">
          <div className="space-y-2">
            <Label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest ml-1">หน่วยงานที่รับผิดชอบ</Label>
            {isEditingInfo ? (
              <Input value={projectDetail.agency} onChange={(e) => setProjectDetail({...projectDetail, agency: e.target.value})} className="h-12 rounded-full border-[#D1CDC7] bg-white px-4" />
            ) : (
              <div className="flex items-center gap-2.5 font-semibold text-[#191c20] px-1">
                <Building2 className="w-5 h-5 text-[#00734b]" /> {projectDetail.agency}
              </div>
            )}
          </div>
          <div className="space-y-2">
            <Label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest ml-1">ปีงบประมาณ</Label>
            {isEditingInfo ? (
              <Input type="number" value={projectDetail.fiscalYear} onChange={(e) => setProjectDetail({...projectDetail, fiscalYear: parseInt(e.target.value)})} className="h-12 rounded-full border-[#D1CDC7] bg-white px-4 w-40" />
            ) : (
              <div className="flex items-center gap-2.5 font-semibold text-[#191c20] px-1">
                <CalendarDays className="w-5 h-5 text-[#00734b]" /> พ.ศ. {projectDetail.fiscalYear}
              </div>
            )}
          </div>
        </div>
      ...
      {!projectDetail.hasProposal && (
        <div className="bg-orange-50/80 border border-status-orange/30 p-8 rounded-[40px] flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-sm">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h3 className="font-extrabold text-[#191c20] text-lg">ขั้นตอนถัดไป: จัดทำข้อเสนอโครงการ</h3>
              <p className="text-[15px] text-slate-600 mt-1">
                กรุณากรอกแบบฟอร์ม 5 หมวดหลักให้ครบถ้วน เพื่อยื่นพิจารณา
              </p>
            </div>
          </div>
          <Button onClick={() => router.push(`/projects/${projectId}/proposal/create`)} className="font-bold gap-2 bg-status-orange hover:bg-[#d65f00] text-white rounded-full px-8 h-14 shadow-sm shrink-0 w-full md:w-auto text-base">
      ...
      <div className="space-y-6 mt-4">
        <div className="px-2">
          <h2 className="text-xl font-extrabold text-[#191c20] flex items-center gap-2">
            เอกสารประกอบโครงการ <span className="text-status-orange text-lg">*</span>
          </h2>
        ...
        <Card className="rounded-[40px] border-[#D1CDC7] shadow-sm bg-white overflow-hidden">
          ...
          <CardContent className="p-0">
            <div className="flex flex-col divide-y divide-[#ededf4]">
              <MandatoryDocRow title="Presentation (PowerPoint)" accept=".ppt,.pptx" file={presentation} onRemove={() => removeFile(setPresentation)} />
              <MandatoryDocRow title="ใบเสนอราคา (PDF)" accept=".pdf" file={quotation} onRemove={() => removeFile(setQuotation)} />
              <MandatoryDocRow title="สรุปโครงการ One Page (PDF)" accept=".pdf" file={onePage} onRemove={() => removeFile(setOnePage)} />
              <MandatoryDocRow title="หนังสือรับรองจาก รผว. (PDF)" accept=".pdf" file={approvalDoc} onRemove={() => removeFile(setApprovalDoc)} />
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {/* ========================================================= */}
      {/* 4. Diagrams (Optional - Grid View) */}
      {/* ========================================================= */}
      <div className="space-y-6 mt-4">
        <div className="px-2 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-extrabold text-[#191c20]">สถาปัตยกรรมและรูปภาพระบบ</h2>
            <p className="text-sm text-slate-500 mt-1">อัปโหลดภาพประกอบระบบเพิ่มเติม <span className="font-semibold text-slate-400">(ทางเลือก)</span></p>
          </div>
        </div>
        ...
        <Card className="rounded-[40px] border-[#D1CDC7] shadow-sm bg-white overflow-hidden">
          <CardHeader className="border-b border-[#ededf4] px-8 py-2 flex flex-row items-center justify-between rounded-none">
            ...
            <span className="bg-white text-slate-600 px-4 py-1.5 rounded-full border border-[#D1CDC7] text-xs font-bold shadow-sm">
              อัปโหลดแล้ว {diagrams.length} / 4
            </span>
          </CardHeader>
          <CardContent className="p-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {diagrams.map(doc => (
                <DiagramPreviewCard key={doc.id} file={doc} onRemove={() => removeFile(setDiagrams, doc.id)} />
              ))}
              {diagrams.length < 4 && <DiagramUploadPlaceholder />}
            </div>
          </CardContent>
        </Card>
      </div>
